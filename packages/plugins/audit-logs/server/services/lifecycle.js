    ...
    async bootstrap() {
      ...
      lifecycleSubscription = strapi.db.lifecycles.subscribe({
        ...
        async beforeUpdate(event) {
          ...
          // Store the data before update in the event state for comparison
          const recordId = params.where?.id || params.where?.documentId;
          
          if (recordId) {
            try {
              const dataBeforeUpdate = await strapi.db.query(model.uid).findOne({
                where: { id: recordId },
              });
              
              // Store in event state for use in afterUpdate
              event.state = event.state || {};
              event.state.dataBeforeUpdate = dataBeforeUpdate;
            } catch (error) {
              strapi.log.debug('Could not fetch data before update for audit log:', error);
            }
          }
        },
        ...
        async afterUpdate(event) {
          ...
          const user = auditLogsService.getCurrentUser();
          const previousData = state?.dataBeforeUpdate;
