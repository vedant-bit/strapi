...
module.exports = ({ strapi }) => ({
  ...
  async saveAuditLog(logData) {
    try {
      const auditLog = await strapi.db.query('api::audit-log.audit-log').create({
        data: {
          ...logData,
          timestamp: new Date(),
        },
  ...
  async findAuditLog(id) {
    try {
      return await strapi.db.query('api::audit-log.audit-log').findOne({
        where: { id },
      });
    } catch (error) {
  ...
  async deleteExpiredLogs(expirationDate) {
    try {
      const result = await strapi.db.query('api::audit-log.audit-log').deleteMany({
        where: {
          timestamp: {
            $lt: expirationDate,
          },
  ...
  shouldAuditContentType(uid) {
    ...
    if (uid === 'api::audit-log.audit-log') {
      return false;
    }
